...
import com.example.data.medicine.local.entity.MedicineEntity.Companion.toEntity
import com.example.data.medicine.local.entity.MedicineEntity.Companion.toModel
...
internal class MedicineRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllMedicines(): Flow<ResponseState<MedicineList>> {
        return flow {
            val list = medicineDao.findAll().map { it.toModel() }
                ...
                ApiResponseHandler().handle {
                    ...
                    when(result) {
                        is ApiResponse.Success -> {
                            ...
                            // DB에 저장
                            medicineDao.insertAll(data.medicines.map { it.toEntity() })
